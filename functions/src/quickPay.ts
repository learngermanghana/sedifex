...
function normalizeDoc(
  ...
  const type = normalizeType(
    record.type ?? record.item_type ?? record.itemType,
    fallbackType,
  );
...
function formatEventTime(startAt: unknown, endAt: unknown, fallback: unknown) {
  const fallbackText = cleanText(fallback, 160);
...
function normalizeAvailabilitySlot(
  ...
  if (
    ...
    record.visibleOnWebsite === false
  )
    return null;
  const name = getName(record);
  if (!name) return null;
  const eventDate = cleanText(record.eventDate, 40);
  const dateLabel =
    cleanText(record.displayDateText, 160) ||
    (eventDate
      ? formatEventDate(`${eventDate}T00:00:00`)
      : formatEventDate(record.startAt));
  const timeLabel = formatEventTime(
    record.startAt,
    record.endAt,
    record.displayTimeText,
  );
  ...
  return {
    ...
    description:
      [
        ...
        .filter(Boolean)
        .join("\n") || null,
    imageUrl: cleanText(record.imageUrl ?? attributes.imageUrl, 2000) || null,
    category: cleanText(record.eventKind, 180) || "Upcoming event",
  };
}
...
function normalizeStore(
  ...
  return {
    ...
    logoUrl:
      cleanText(
        record.logoUrl ?? record.logo ?? record.photoUrl ?? record.imageUrl,
        2000,
      ) || null,
...
function storeMatchesQuery(store: PublicStore, query: string) {
  ...
  const haystack = normalizeSearchText(
    [store.name, store.city ?? "", store.phone ?? ""].join(" "),
  );
...
async function fetchQueryItems(
  collection: string,
  storeId: string,
  fallbackType: CatalogType,
) {
  ...
  return snapshot.docs
    .map((doc) =>
      normalizeDoc(doc.id, doc.data() as Record<string, unknown>, fallbackType),
    )
    .filter((item): item is CatalogItem => item !== null);
}
...
async function fetchIndexedStores(query: string) {
  ...
  return snapshot.docs
    .map((doc) =>
      normalizeStore(
        cleanText(doc.data().storeId, 180) || doc.id,
        doc.data() as Record<string, unknown>,
      ),
    )
...
export const publicQuickPayStores = functions.https.onRequest(
  async (req, res) => {
    setCors(res);
    ...
    try {
      ...
      if (normalizedQuery.length < 2) {
        ...
        stores = snapshot.docs
          .map((doc) =>
            normalizeStore(doc.id, doc.data() as Record<string, unknown>),
          )
          .filter((store): store is PublicStore => store !== null);
        ...
        stores = indexedStores.length
          ? indexedStores
          : await fetchFallbackStores(query);
      ...
      res.status(200).json({
        ok: true,
        count: stores.length,
        stores: stores.slice(0, MAX_STORE_RESULTS),
        source,
      });
    } catch (error) {
...
export const publicQuickPayCatalog = functions.https.onRequest(
  async (req, res) => {
    setCors(res);
    ...
    const [
      ...
      fetchCollectionItems(
        `stores/${storeId}/integrationAvailabilitySlots`,
        "BOOKING",
      ),
