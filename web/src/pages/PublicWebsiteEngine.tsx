...
type PublicPage = 'home' | 'products' | 'services' | 'gallery' | 'quick-pay' | 'contact'
type WebsiteSettings = { storeId: string; businessName: string; slug: string; websiteType: string; theme: string; pages: string[]; status: 'draft' | 'published'; tagline: string; description: string; brandColor: string; coverImageUrl: string }
type StoreProfile = { name: string; logoUrl: string; phone: string; whatsapp: string; email: string; address: string; openingHours: string }
type PublicItem = { id: string; name: string; price: number; type: 'PRODUCT' | 'SERVICE' | 'COURSE'; description: string; imageUrl: string }
...
function money(value: number) { return new Intl.NumberFormat('en-GH', { style: 'currency', currency: 'GHS' }).format(value) }
function getPrice(source: Record<string, unknown>) { const minor = numberValue(source.priceMinor ?? source.amountMinor); if (minor !== null && minor >= 0) return minor / 100; const major = numberValue(source.price ?? source.sellingPrice ?? source.salePrice ?? source.amount ?? source.fee); return major !== null && major >= 0 ? major : 0 }
function mapItem(id: string, source: Record<string, unknown>, fallbackType: PublicItem['type']): PublicItem | null { const name = clean(source.name ?? source.productName ?? source.serviceName ?? source.courseName ?? source.title, 220); if (!name) return null; const rawType = clean(source.type ?? source.item_type ?? source.itemType, 40).toUpperCase(); const type: PublicItem['type'] = rawType === 'SERVICE' || rawType === 'COURSE' || rawType === 'PRODUCT' ? rawType : fallbackType; return { id, name, type, price: getPrice(source), description: clean(source.description ?? source.shortDescription, 260), imageUrl: clean(source.imageUrl ?? source.image_url ?? source.image ?? source.photoUrl ?? source.coverImageUrl, 900) } }
function mapGallery(id: string, source: Record<string, unknown>): GalleryItem | null { const url = clean(source.url ?? source.imageUrl ?? source.image_url ?? source.image ?? source.photoUrl, 900); if (!url) return null; return { id, url, alt: clean(source.alt ?? source.imageAlt ?? source.caption, 140) || 'Gallery image', caption: clean(source.caption ?? source.title ?? source.description, 220) } }
function themeClasses(theme: string) { if (theme === 'luxury') return 'from-stone-950 via-amber-950 to-slate-950'; if (theme === 'bold') return 'from-indigo-950 via-fuchsia-950 to-slate-950'; if (theme === 'clean') return 'from-slate-900 via-slate-800 to-slate-950'; return 'from-slate-950 via-indigo-950 to-slate-900' }
function normalizePage(value?: string): PublicPage { const page = (value || 'home').toLowerCase().replace(/\s+/g, '-'); if (page.includes('product') || page.includes('categor') || page.includes('cart') || page.includes('checkout')) return 'products'; if (page.includes('service') || page.includes('booking') || page.includes('course') || page.includes('class')) return 'services'; if (page.includes('gallery')) return 'gallery'; if (page.includes('quick') || page.includes('pay')) return 'quick-pay'; if (page.includes('contact')) return 'contact'; return 'home' }
function pagePath(slug: string, page: PublicPage) { return page === 'home' ? `/sites/${slug}` : `/sites/${slug}/${page}` }
function pageLabel(page: PublicPage) { return page === 'quick-pay' ? 'Quick Pay' : page.charAt(0).toUpperCase() + page.slice(1) }
function pageFromLabel(label: string): PublicPage | null { const value = label.toLowerCase(); if (value.includes('home')) return 'home'; if (value.includes('product') || value.includes('categor') || value.includes('cart') || value.includes('checkout')) return 'products'; if (value.includes('service') || value.includes('booking') || value.includes('course') || value.includes('class')) return 'services'; if (value.includes('gallery')) return 'gallery'; if (value.includes('quick') || value.includes('pay')) return 'quick-pay'; if (value.includes('contact')) return 'contact'; return null }
function uniquePages(labels: string[]): PublicPage[] { const pages: PublicPage[] = ['home']; labels.forEach(label => { const page = pageFromLabel(label); if (page && !pages.includes(page)) pages.push(page) }); if (!pages.includes('contact')) pages.push('contact'); return pages }
...
export default function PublicWebsiteEngine() {
  ...
  useEffect(() => {
    ...
    async function loadWebsite() {
      try {
        ...
        const websiteSnap = await getDocs(query(collection(db, 'storeSettings'), where('websiteBuilder.slug', '==', slug), where('websiteBuilder.status', '==', 'published'), limit(1)))
        if (websiteSnap.empty) { if (mounted) setError('This website is not published yet.'); return }
        ...
        const profileData: StoreProfile = { name: clean(publicProfile.displayName ?? website.businessName ?? storeData.businessName ?? storeData.storeName ?? storeData.name, 220) || slug, logoUrl: clean(publicProfile.logoUrl ?? website.businessLogoUrl ?? storeData.logoUrl ?? storeData.logo ?? storeData.photoUrl, 900), phone: clean(publicProfile.publicPhone ?? website.phone ?? storeData.phone ?? storeData.businessPhone ?? storeData.phoneNumber, 80), whatsapp: clean(publicProfile.whatsappNumber ?? website.whatsapp ?? storeData.whatsapp ?? storeData.whatsappNumber, 80), email: clean(publicProfile.publicEmail ?? website.email ?? storeData.email ?? storeData.businessEmail, 220), address: clean(publicProfile.addressLine1 ?? website.location ?? storeData.address ?? storeData.addressLine1 ?? storeData.town ?? storeData.city, 260), openingHours: clean(publicProfile.openingHours ?? website.openingHours ?? storeData.openingHours, 120) }
        const specs: Array<{ path: string; type: PublicItem['type'] }> = [{ path: `stores/${storeId}/products`, type: 'PRODUCT' }, { path: `stores/${storeId}/services`, type: 'SERVICE' }, { path: `stores/${storeId}/courses`, type: 'COURSE' }]
        const itemGroups = await Promise.all(specs.map(async spec => { const snap = await getDocs(query(collection(db, spec.path), limit(40))); return snap.docs.map(itemDoc => mapItem(itemDoc.id, itemDoc.data() as Record<string, unknown>, spec.type)).filter((item): item is PublicItem => Boolean(item)) }))
        const gallerySnap = await getDocs(query(collection(db, 'stores', storeId, 'promoGallery'), limit(40)))
        const galleryItems = gallerySnap.docs.map(itemDoc => mapGallery(itemDoc.id, itemDoc.data() as Record<string, unknown>)).filter((item): item is GalleryItem => Boolean(item))
        if (!mounted) return
        setSettings({ storeId, businessName: profileData.name, slug, websiteType: clean(website.websiteType, 40) || 'shop', theme: clean(website.theme, 40) || 'modern', pages: Array.isArray(website.pages) ? website.pages.filter((page): page is string => typeof page === 'string') : ['Home', 'Products', 'Services', 'Gallery', 'Contact', 'Quick Pay'], status: 'published', tagline: clean(publicProfile.tagline ?? website.tagline ?? storeData.tagline, 180), description: clean(publicProfile.businessDescription ?? website.description ?? storeData.description, 600), brandColor: /^#[0-9a-f]{6}$/i.test(String(website.brandColor || publicProfile.brandColor || storeData.brandColor || '')) ? String(website.brandColor || publicProfile.brandColor || storeData.brandColor) : '#4f46e5', coverImageUrl: clean(publicProfile.coverImageUrl ?? website.coverImageUrl ?? storeData.coverImageUrl ?? storeData.bannerImageUrl, 900) })
        setProfile(profileData); setItems(itemGroups.flat()); setGallery(galleryItems)
      } catch (loadError) { console.error('[public-website] Unable to load website', loadError); if (mounted) setError('Unable to load this website.') } finally { if (mounted) setLoading(false) }
    }
    void loadWebsite()
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-950"><header className="sticky top-0 z-30 border-b border-white/10 bg-slate-950/90 text-white backdrop-blur"><div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8"><Link to={pagePath(slug, 'home')} className="flex items-center gap-3 text-white no-underline">{profile.logoUrl ? <img src={profile.logoUrl} alt={`${profile.name} logo`} className="h-11 w-11 rounded-2xl object-cover" /> : <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white/10 font-black">{profile.name.slice(0, 1)}</div>}<span className="font-black">{profile.name}</span></Link><nav className="flex flex-wrap items-center gap-2 text-sm">{availablePages.map(page => <Link key={page} className={`rounded-full px-4 py-2 font-semibold no-underline ${activePage === page ? 'bg-white text-slate-950' : 'text-white/80 hover:bg-white/10 hover:text-white'}`} to={pagePath(slug, page)}>{pageLabel(page)}</Link>)}</nav></div></header>
    {activePage === 'home' ? <><section className={`bg-gradient-to-br ${themeClasses(settings.theme)} px-4 py-24 text-white sm:px-6 lg:px-8`} style={heroStyle}><div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center"><div><p className="text-sm font-semibold uppercase tracking-[0.3em] text-cyan-200">{settings.websiteType} website</p><h1 className="mt-5 text-5xl font-black tracking-tight sm:text-7xl">{settings.tagline || profile.name}</h1><p className="mt-5 max-w-2xl text-lg leading-8 text-slate-200">{settings.description || 'Discover our products, services, bookings, and secure payments — powered by Sedifex.'}</p><div className="mt-8 flex flex-wrap gap-3"><Link className="rounded-2xl bg-white px-6 py-3 font-semibold text-slate-950 no-underline" to={pagePath(slug, 'products')}>View products</Link><Link className="rounded-2xl border border-white/20 px-6 py-3 font-semibold text-white no-underline" to={pagePath(slug, 'contact')}>Contact us</Link></div></div><div className="rounded-[2rem] border border-white/10 bg-white/10 p-6 shadow-2xl backdrop-blur"><h2 className="text-2xl font-black">Explore this website</h2><div className="mt-5 grid gap-3 text-sm text-slate-200">{availablePages.map(page => <Link key={page} to={pagePath(slug, page)} className="rounded-2xl bg-white/10 p-4 font-semibold text-white no-underline hover:bg-white/20">{pageLabel(page)}</Link>)}</div></div></div></section>{productItems.length ? <ProductsSection title="Featured products" items={productItems.slice(0, 4)} quickPayUrl={quickPayUrl} ctaHref={pagePath(slug, 'products')} /> : null}{serviceItems.length ? <ServicesSection title="Featured services" items={serviceItems.slice(0, 3)} quickPayUrl={quickPayUrl} ctaHref={pagePath(slug, 'services')} /> : null}{gallery.length ? <GallerySection gallery={gallery.slice(0, 6)} ctaHref={pagePath(slug, 'gallery')} /> : null}<QuickPayBlock quickPayUrl={quickPayUrl} /></> : null}
