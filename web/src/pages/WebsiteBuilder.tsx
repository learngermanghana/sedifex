...
const WebsiteBuilderAssistantPanel = React.lazy(
  () => import("./WebsiteBuilderAssistantPanel"),
);
...
type TemplateSelection = {
  ...
  seoSettings: { title: string; metaDescription: string; keywords: string };
};
type DomainSettings = {
  customDomain: string;
...
const BUILDER_STEPS: Array<{
...
}> = [
  {
    ...
    description: "Name, logo, contact details, social links, and brand assets.",
  },
  {
    id: "type",
    label: "Website type",
    description: "Choose the right structure for the business.",
  },
  {
    id: "pages",
    label: "Pages",
    description: "Select smart pages based on the website type.",
  },
  {
    id: "theme",
    label: "Theme",
    description: "Pick the visual style and feel.",
  },
  {
    id: "content",
    label: "Content",
    description: "Generate first-draft website copy from business data.",
  },
  {
    id: "seo",
    label: "SEO settings",
    description: "Control Google and social share previews.",
  },
  {
    id: "payments",
    label: "Payments / Quick Pay",
    description: "Prepare checkout and payment pages.",
  },
  {
    id: "domain",
    label: "Domain",
    description: "Set free URL, custom domain, DNS, and SSL.",
  },
  {
    id: "publish",
    label: "Publish",
    description: "Review checklist, publish, and share the site.",
  },
];
...
const WEBSITE_TYPES: WebsiteTypeOption[] = [
  {
    id: "shop",
    label: "Shop website",
    description: "Products, categories, checkout, cart, and Quick Pay.",
    icon: "🛍️",
  ...
  },
  {
    ...
    description: "Services, bookings, gallery, client reviews, and Quick Pay.",
    icon: "✨",
  ...
  },
  {
    ...
    description: "Courses, registration, classes, and student payments.",
    icon: "🎓",
  ...
  },
  {
    ...
    description:
      "Packages, destinations, enquiries, consultations, and bookings.",
    icon: "✈️",
  ...
  },
  {
    id: "ngo",
    label: "NGO website",
    description: "Programs, donations, volunteers, blog, and impact gallery.",
    icon: "🤝",
  ...
  },
  {
    ...
    description: "Menu, ordering, reservations, table QR, and payments.",
    icon: "🍽️",
  ...
  },
  {
    ...
    description: "Services, bookings, invoices, testimonials, and Quick Pay.",
    icon: "🧰",
...
const CONTENT_GENERATOR_PROFILE: Record<WebsiteType, ContentGeneratorProfile> =
  {
    ngo: {
      noun: "foundation",
      offerPlural:
        "programs, donations, volunteer work, and impact-driven projects",
      audience: "communities and supporters",
      promise: "create lasting impact where support is needed most",
      action: "Support the mission",
      seoLabel: "Programs, Donations & Volunteer Support",
      tagline: "Supporting communities through action and impact",
      serviceIntro:
        "Our programs are designed to support communities, mobilize volunteers, and turn donations into measurable impact.",
      keywords: [
        "community programs",
        "donations",
        "volunteer support",
    ...
    shop: {
      ...
      seoLabel: "Products, Categories & Online Checkout",
      tagline: "Shop quality products with easy online payment",
      serviceIntro:
        "Our product range is organized to help customers find what they need, compare options, and complete checkout with confidence.",
      keywords: [
        "online shop",
        "products",
        "categories",
    ...
    beauty: {
      ...
      seoLabel: "Beauty Services, Salon Bookings & Client Reviews",
      tagline: "Beauty services designed around confidence and care",
      serviceIntro:
        "Our services are created to help clients look good, feel confident, and book professional beauty treatments with ease.",
      keywords: [
        "beauty services",
        "salon booking",
        "client reviews",
    ...
    school: {
      ...
      seoLabel: "Courses, Registration, Classes & Student Payments",
      tagline: "Learn, register, and manage classes with ease",
      serviceIntro:
        "Our courses and classes are organized to help students learn clearly, register easily, and stay connected to school updates.",
      keywords: [
        "courses",
        "registration",
        "classes",
    ...
    travel: {
      noun: "travel agency",
      offerPlural:
        "packages, destinations, consultations, bookings, and travel enquiries",
      ...
      seoLabel: "Travel Packages, Destinations & Bookings",
      tagline: "Clear travel planning from enquiry to booking",
      serviceIntro:
        "Our travel support helps customers explore packages, compare destinations, request consultation, and move from enquiry to booking.",
      keywords: [
        "travel packages",
        "destinations",
        "bookings",
    ...
    restaurant: {
      noun: "restaurant",
      offerPlural:
        "menu items, online ordering, reservations, table QR, and payments",
      ...
      seoLabel: "Menu, Online Ordering & Reservations",
      tagline: "Great food with simple ordering and reservations",
      serviceIntro:
        "Our menu and ordering options help customers browse meals, place orders, reserve tables, and pay with less stress.",
      keywords: [
        "restaurant menu",
        "online ordering",
        "reservations",
    ...
    service: {
      ...
      seoLabel: "Services, Bookings, Invoices & Quick Pay",
      tagline: "Reliable services with simple booking and payment",
      serviceIntro:
        "Our service packages are built to help customers understand what we offer, request support, book appointments, and pay easily.",
      keywords: [
        "services",
        "bookings",
        "invoices",
...
function getDisplayStatus(
  ...
  if (!storeId || !settings.slug || settings.pages.length === 0)
    return "needs-setup";
...
function isDomainStatus(value: unknown): value is DomainConnectionStatus {
  return ["not-connected", "pending", "connected", "needs-attention"].includes(
    String(value),
  );
}
function isSslStatus(value: unknown): value is SslStatus {
  return ["not-started", "pending", "active", "failed"].includes(String(value));
}
...
function mergeSeoSettings(...sources: unknown[]): SeoSettings {
  ...
  sources.forEach((source) => {
    const record = getRecord(source);
    (
      ["title", "metaDescription", "keywords", "socialShareImage"] as Array<
        keyof SeoSettings
      >
    ).forEach((key) => {
      const value = record[key];
...
function mergeDomainSettings(...sources: unknown[]): DomainSettings {
  ...
  sources.forEach((source) => {
    ...
    if (isDomainStatus(record.connectionStatus))
      merged.connectionStatus = record.connectionStatus;
    if (isSslStatus(record.sslStatus)) merged.sslStatus = record.sslStatus;
  });
  return merged;
}

function buildSeoTitle(settings: WebsiteBuilderSettings) {
  const profile = CONTENT_GENERATOR_PROFILE[settings.websiteType];
  const businessName = settings.businessName.trim() || "Business website";
  return `${businessName} | ${profile.seoLabel}`;
}
function buildSeoDescription(settings: WebsiteBuilderSettings) {
  const profile = CONTENT_GENERATOR_PROFILE[settings.websiteType];
  ...
  if (settings.websiteType === "ngo")
    return `Support ${businessName} through donations, volunteer programs, and community impact projects${locationPhrase}.`;
  return `${businessName} helps ${profile.audience}${locationPhrase} through ${profile.offerPlural}. ${profile.action} and connect with us online.`;
}
function buildSeoKeywords(settings: WebsiteBuilderSettings) {
  const profile = CONTENT_GENERATOR_PROFILE[settings.websiteType];
  return [
...
function buildVerificationToken(storeId: string | null, slug: string) {
  return `sedifex-site-verification=${storeId || slug || "your-site"}`;
}
function buildQrCodeUrl(url: string, size = 260) {
  return `https://api.qrserver.com/v1/create-qr-code/?size=${size}x${size}&format=png&data=${encodeURIComponent(url)}`;
}
function buildShareText(settings: WebsiteBuilderSettings, publicUrl: string) {
  const name = settings.businessName.trim() || "Our business";
  return `Visit ${name} online: ${publicUrl}`;
}

function buildGeneratedContent(
  settings: WebsiteBuilderSettings,
  key: ContentDraftKey | "tagline",
) {
  const profile = CONTENT_GENERATOR_PROFILE[settings.websiteType];
  const businessName = settings.businessName.trim() || "This business";
  const location = settings.location.trim();
  const locationPhrase = location ? ` in ${location}` : "";
  const selectedPages = settings.pages.length
    ? settings.pages.join(", ")
    : PAGE_OPTIONS_BY_TYPE[settings.websiteType].join(", ");
...
export default function WebsiteBuilder() {
  ...
  const previewPages = settings.pages.length
    ? settings.pages.slice(0, 5)
    : ["Home"];
  const activeSocialLinks = SOCIAL_LINK_FIELDS.filter(
    (field) => settings.socialLinks[field.id],
  ).slice(0, 4);
  const hasPaymentPage = paymentPages.some((page) =>
    settings.pages.includes(page),
  );
  const seoTitlePreview =
    settings.seoSettings.title ||
    settings.contentDrafts.seoTitle ||
    buildSeoTitle(settings);
  const seoDescriptionPreview =
    settings.seoSettings.metaDescription || buildSeoDescription(settings);
  const seoShareImage =
    settings.seoSettings.socialShareImage ||
    settings.coverImageUrl ||
    settings.businessLogoUrl;
  const domainStatusConfig =
    DOMAIN_STATUS_CONFIG[settings.domainSettings.connectionStatus];
  ...
  const verificationToken =
    settings.domainSettings.verificationToken ||
    buildVerificationToken(storeId, settings.slug);
  ...
  const dnsRecords = [
    {
      type: "CNAME",
      host: customDomain.startsWith("www.") ? "www" : "@ or www",
      value: settings.domainSettings.dnsTarget,
      note: "Points the custom domain to Sedifex Sites.",
    },
    {
      type: "TXT",
      host: "_sedifex",
      value: verificationToken,
      note: "Verifies that this business owns the domain.",
    },
  ];
  const publishChecklist: PublishChecklistItem[] = [
    {
      ...
      complete: Boolean(settings.businessName.trim()),
      helper: "Add the official business name in Business identity.",
    },
    {
      id: "phone",
      label: "Phone or WhatsApp added",
      complete: Boolean(settings.phone.trim() || settings.whatsapp.trim()),
      helper:
        "Add at least one contact number so customers can reach the business.",
    },
    {
      id: "pages",
      label: "At least one page selected",
      complete: settings.pages.length > 0,
      helper: "Select pages for the public website.",
    },
    {
      id: "brandAssets",
      label: "Logo or banner added",
      complete: Boolean(
        settings.businessLogoUrl.trim() || settings.coverImageUrl.trim(),
      ),
      helper: "Add a logo or cover/banner image for stronger branding.",
    },
    {
      id: "payment",
      label: "Payment / Quick Pay configured if enabled",
      complete: hasPaymentPage,
      helper: `Enable at least one recommended action page: ${paymentPages.join(", ")}.`,
    },
    {
      id: "offerings",
      label: "Products/services/programs available",
      complete:
        offeringDataAvailable ||
        Boolean(settings.contentDrafts.serviceDescriptions.trim()),
      helper: isCheckingOfferings
        ? "Checking Sedifex data…"
        : "Add products/services/programs in Sedifex or generate descriptions as a starter.",
    },
    {
      id: "domain",
      label: "Domain ready or free URL ready",
      complete:
        Boolean(settings.slug.trim()) ||
        settings.domainSettings.connectionStatus === "connected",
      helper:
        "The free Sedifex URL is enough to publish. Custom domain can be connected later.",
    },
  ];
  ...
  const publishSummary =
    missingPublishItems.length === 0
      ? "Ready to publish"
      : `${missingPublishItems.length} things missing before publishing`;
  const canPreview = true;
  const canPublish = Boolean(
    storeId && !isLoading && !isSaving && missingPublishItems.length === 0,
  );
  const currentStepIndex = BUILDER_STEPS.findIndex(
    (step) => step.id === activeStep,
  );
  const safeStepIndex = currentStepIndex >= 0 ? currentStepIndex : 0;
  const currentStep = BUILDER_STEPS[safeStepIndex];
  const progressPercent = Math.round(
    ((safeStepIndex + 1) / BUILDER_STEPS.length) * 100,
  );
  ...
  const stepCompletion: Record<BuilderStepId, boolean> = {
    ...
    seo: Boolean(
      settings.seoSettings.title && settings.seoSettings.metaDescription,
    ),
    ...
    async function checkOfferingData() {
      ...
      try {
        const checks = await Promise.all(
          OFFERING_COLLECTIONS.map(async (name) => {
            try {
              ...
              console.info(
                `[website-builder] Top-level ${name} check unavailable`,
                topLevelError,
              );
            }
            try {
              const nestedSnap = await getDocs(
                query(collection(db, "stores", storeId, name), limit(1)),
              );
              return !nestedSnap.empty;
            } catch (nestedError) {
              console.info(
                `[website-builder] Nested ${name} check unavailable`,
                nestedError,
              );
              return false;
            }
    ...
    async function loadSettings() {
      ...
      try {
        ...
        setSettings((previous) => {
          ...
          const websiteType = isWebsiteType(website.websiteType)
            ? website.websiteType
            : previous.websiteType;
          const loadedPages = Array.isArray(website.pages)
            ? website.pages.filter(
                (page): page is string => typeof page === "string",
              )
            : [];
          const pages = loadedPages.length
            ? filterPagesForType(websiteType, loadedPages)
            : [...PAGE_OPTIONS_BY_TYPE[websiteType]];
          const contentDrafts = mergeContentDrafts(website.contentDrafts);
          const seoSettings = mergeSeoSettings(website.seoSettings, {
            title: readString(website, "seoTitle", contentDrafts.seoTitle),
            socialShareImage: readString(website, "socialShareImage"),
          });
          const domainSettings = mergeDomainSettings(website.domainSettings, {
            customDomain: readString(website, "customDomain"),
          });
          return {
            ...previous,
            slug: readString(website, "slug") || slugify(businessName),
            websiteType,
            businessType: readString(
              website,
              "businessType",
              WEBSITE_TYPES.find((type) => type.id === websiteType)?.label ||
                previous.businessType,
            ),
            theme: isWebsiteTheme(website.theme)
              ? website.theme
              : previous.theme,
            selectedTemplateId: readString(website, "selectedTemplateId"),
            selectedTemplateName: readString(website, "selectedTemplateName"),
            ...
            tagline: readString(
              website,
              "tagline",
              readString(storeData, "tagline"),
            ),
            description: readString(
              website,
              "description",
              readString(storeData, "description"),
            ),
            phone: readString(
              website,
              "phone",
              readString(
                storeData,
                "phone",
                readString(storeData, "phoneNumber"),
              ),
            ),
            whatsapp: readString(
              website,
              ...
              readString(
                storeData,
                "whatsapp",
                readString(storeData, "whatsappNumber"),
              ),
            ),
            email: readString(
              website,
              ...
              readString(
                storeData,
                "email",
                readString(storeData, "businessEmail"),
              ),
            ),
            location: readString(
              website,
              ...
              readString(
                storeData,
                "location",
                readString(storeData, "address"),
              ),
            ),
            openingHours: readString(
              website,
              "openingHours",
              readString(storeData, "openingHours"),
            ),
            businessLogoUrl: readString(
              website,
              "businessLogoUrl",
              readString(
                storeData,
                "businessLogoUrl",
                readString(storeData, "logoUrl"),
              ),
            ),
            coverImageUrl: readString(
              website,
              "coverImageUrl",
              readString(
                storeData,
                "coverImageUrl",
                readString(storeData, "bannerImageUrl"),
              ),
            ),
            brandColor: normalizeBrandColor(
              website.brandColor || storeData.brandColor,
            ),
  ...
  function updateSetting<K extends keyof WebsiteBuilderSettings>(
    key: K,
    value: WebsiteBuilderSettings[K],
  ) {
  ...
  function updateContentDraft(key: ContentDraftKey, value: string) {
    setSettings((previous) => ({
      ...previous,
      contentDrafts: { ...previous.contentDrafts, [key]: value },
    }));
  }
  function updateSeoSetting(key: keyof SeoSettings, value: string) {
    setSettings((previous) => ({
      ...previous,
      seoSettings: { ...previous.seoSettings, [key]: value },
    }));
  }
  function updateDomainSetting<K extends keyof DomainSettings>(
    key: K,
    ...
    setSettings((previous) => ({
      ...previous,
      domainSettings: {
        ...previous.domainSettings,
        [key]: key === "customDomain" ? normalizeDomain(String(value)) : value,
      },
    }));
  }
  ...
  function updateSocialLink(key: keyof SocialLinks, value: string) {
    setSettings((previous) => ({
      ...previous,
      socialLinks: { ...previous.socialLinks, [key]: value },
    }));
  }
  ...
  function generateContent(key: ContentDraftKey | "tagline") {
    ...
    setFeedback(
      `${key === "serviceDescriptions" ? "Service descriptions" : key} generated.`,
    );
  }
  ...
  function generateSeoSettings() {
    ...
    setSettings((previous) => ({
      ...previous,
      contentDrafts: { ...previous.contentDrafts, seoTitle: title },
      seoSettings: {
        ...previous.seoSettings,
        title,
        metaDescription,
        keywords,
        socialShareImage,
  ...
  function generateAllContent() {
    ...
    const serviceDescriptions = buildGeneratedContent(
      settings,
      "serviceDescriptions",
    );
    ...
    setSettings((previous) => ({
      ...
      seoSettings: {
        ...previous.seoSettings,
        title: previous.seoSettings.title.trim()
          ? previous.seoSettings.title
          : seoTitle,
        metaDescription: previous.seoSettings.metaDescription.trim()
          ? previous.seoSettings.metaDescription
          : metaDescription,
        keywords: previous.seoSettings.keywords.trim()
          ? previous.seoSettings.keywords
          : keywords,
        socialShareImage:
          previous.seoSettings.socialShareImage ||
  ...
  async function copyText(
    ...
    try {
      ...
      console.error("[website-builder] Unable to copy text", copyError);
      setMessage("Could not copy. Please copy manually.");
    }
    window.setTimeout(() => setMessage(null), 2500);
  }

  async function copyDnsRecords() {
    await copyText(
      dnsRecords
  ...
  async function copyWebsiteLink() {
    await copyText(publicWebsiteUrl, "Website link copied.", setCopyFeedback);
  }
  ...
  function saveDraftToLocalPreview(nextSettings: WebsiteBuilderSettings) {
    if (typeof window === "undefined") return;
    const payload = { savedAt: Date.now(), data: nextSettings };
    try {
      window.sessionStorage.setItem(
        previewDraftStorageKey(storeId),
      ...
      console.info(
        "[website-builder] Unable to persist preview draft locally",
        storageError,
      );
    }
  }
  ...
  async function saveSettings(nextStatus: StoredWebsiteStatus) {
    ...
    if (nextStatus === "published" && missingPublishItems.length > 0) {
      setError(publishSummary);
      setActiveStep("publish");
      return;
    }
    const normalizedSlug = slugify(
      settings.slug || settings.businessName || storeId,
    );
    if (!normalizedSlug) {
      setError("Enter a website slug.");
      return;
    }
    const normalizedPages = filterPagesForType(
    ...
    setError(null);
    setFeedback(
      nextStatus === "published" ? "Publishing website…" : "Saving draft…",
    );
    try {
      const normalizedDomain = mergeDomainSettings(settings.domainSettings);
      const finalPublicUrl =
        normalizedDomain.connectionStatus === "connected" &&
        normalizedDomain.customDomain
          ? `https://${normalizedDomain.customDomain}`
          : `https://sites.sedifex.com/${normalizedSlug}`;
      const payload: WebsiteBuilderSettings = {
        ...
        seoSettings: mergeSeoSettings(settings.seoSettings),
        domainSettings: { ...normalizedDomain, verificationToken },
        status: nextStatus,
      };
      ...
      const marketingTools = {
        qrCodeUrl: buildQrCodeUrl(finalPublicUrl),
        shareText: buildShareText(payload, finalPublicUrl),
        whatsappShareUrl: `https://wa.me/?text=${encodeURIComponent(buildShareText(payload, finalPublicUrl))}`,
        facebookShareUrl: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(finalPublicUrl)}`,
        instagramCaption,
      };
      await setDoc(
        doc(db, "storeSettings", storeId),
        {
          websiteBuilder: {
            ...
            domainSettings: payload.domainSettings,
            freeUrl: `https://sites.sedifex.com/${normalizedSlug}`,
            customDomain: payload.domainSettings.customDomain,
            domainConnectionStatus: payload.domainSettings.connectionStatus,
            sslStatus: payload.domainSettings.sslStatus,
            dnsRecords,
            publishChecklist,
            publishChecklistMissing: missingPublishItems.length,
            marketingTools,
      ...
      setSettings(payload);
      saveDraftToLocalPreview(payload);
      setFeedback(
        nextStatus === "published"
          ? "Website published successfully. QR and sharing tools are ready."
          : "Website draft saved.",
      );
    } catch (saveError) {
  ...
  function applyTemplateToDraft(template: TemplateSelection) {
    setSettings((previous) => {
      const draftForTemplate = {
        ...previous,
      ...
      const templateSeoTitle =
        template.seoSettings.title ||
        template.contentDrafts.seoTitle ||
        buildSeoTitle(draftForTemplate);
      const nextSettings: WebsiteBuilderSettings = {
        ...previous,
        ...
        pages: filterPagesForType(template.websiteType, template.pages),
        tagline: template.tagline || previous.tagline,
        description: template.description || previous.description,
        contentDrafts: {
          ...previous.contentDrafts,
          homepage:
            template.contentDrafts.homepage || previous.contentDrafts.homepage,
          about: template.contentDrafts.about || previous.contentDrafts.about,
          serviceDescriptions:
            template.contentDrafts.serviceDescriptions ||
            previous.contentDrafts.serviceDescriptions,
          seoTitle: template.contentDrafts.seoTitle || templateSeoTitle,
        },
        seoSettings: {
          ...previous.seoSettings,
          title: templateSeoTitle,
          metaDescription:
            template.seoSettings.metaDescription ||
            previous.seoSettings.metaDescription,
          keywords:
            template.seoSettings.keywords || previous.seoSettings.keywords,
        },
      };
  ...
  function previewWithMyData() {
    saveDraftToLocalPreview(settings);
    ...
    <PageSection
      ...
      actions={
        <div className="flex flex-wrap items-center gap-3">
          <span
            className={`inline-flex items-center gap-2 rounded-full border px-3 py-2 text-sm font-semibold ${statusConfig.className}`}
          >
            <span
            ...
            <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
              <div>
                ...
                <h3 className="mt-3 text-2xl font-bold tracking-tight md:text-3xl">
                  Publish only when the website is ready
                </h3>
                <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-200">
                  Sedifex now checks the essentials before publishing, then
                  gives every business QR and sharing tools for quick marketing.
                </p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/10 p-4 text-sm shadow-xl backdrop-blur">
                ...
                <p className="mt-1 font-semibold text-white">
                  {settings.businessName || "Loading business…"}
                </p>
              ...
              {BUILDER_STEPS.map((step, index) => {
                ...
                return (
                  <button
                    key={step.id}
                    type="button"
                    className={`rounded-2xl border p-3 text-left transition ${isActive ? "border-indigo-500 bg-indigo-50 shadow-sm ring-2 ring-indigo-100" : "border-slate-200 bg-white hover:border-indigo-200 hover:bg-slate-50"}`}
                    onClick={() => setActiveStep(step.id)}
                  >
                    <span className="flex items-center justify-between gap-2">
                      <span
                        className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold ${isActive ? "bg-indigo-600 text-white" : "bg-slate-100 text-slate-600"}`}
                      >
                        {index + 1}
                      </span>
                      <span
                        className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${isComplete ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}
                      >
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-6">
            {activeStep === "identity" ? (
              <div>
                ...
                <h3 className="mt-1 text-2xl font-semibold text-slate-950">
                  Collect all website identity data
                </h3>
                <p className="mt-2 text-sm text-slate-500">
                  These fields let Sedifex generate a complete website, contact
                  section, footer, social links, and branded preview from one
                  profile.
                </p>
                <div className="mt-6 grid gap-5 lg:grid-cols-2">
                  <label className="block text-sm font-semibold text-slate-700">
                    ...
                    <input
                      ...
                      placeholder="Wesoamo Foundation"
                    />
                  </label>
                  <label className="block text-sm font-semibold text-slate-700">
                    Short tagline
                    <input
                      ...
                      placeholder="Supporting communities through action and impact"
                    />
                  </label>
                  ...
                  <label className="block text-sm font-semibold text-slate-700">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        updateSetting("businessLogoUrl", event.target.value)
                      }
                  ...
                  <label className="block text-sm font-semibold text-slate-700">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        updateSetting("coverImageUrl", event.target.value)
                      }
                  ...
                  <label className="block text-sm font-semibold text-slate-700">
                    Brand color
                    <div className="mt-2 flex items-center gap-3 rounded-2xl border border-slate-300 px-4 py-3 focus-within:border-indigo-500 focus-within:ring-4 focus:ring-indigo-50">
                      <input
                        type="color"
                        className="h-10 w-12 cursor-pointer rounded-lg border border-slate-200 bg-white"
                        value={settings.brandColor}
                        onChange={(event) =>
                          updateSetting("brandColor", event.target.value)
                        }
                      />
                      <input
                        ...
                        onChange={(event) =>
                          updateSetting(
                            "brandColor",
                            normalizeBrandColor(event.target.value),
                          )
                        }
                        placeholder="#4f46e5"
                      />
                  ...
                  <label className="block text-sm font-semibold text-slate-700">
                    ...
                    <div className="mt-2 flex flex-col overflow-hidden rounded-2xl border border-slate-300 bg-white focus-within:border-indigo-500 focus-within:ring-4 focus-within:ring-indigo-50 sm:flex-row">
                      <span className="border-b border-slate-200 bg-slate-50 px-4 py-3 text-slate-500 sm:border-b-0 sm:border-r">
                        sites.sedifex.com/
                      </span>
                      <input
                        ...
                        value={settings.slug}
                        onChange={(event) =>
                          updateSetting("slug", slugify(event.target.value))
                        }
                        placeholder="wesoamo-foundation"
                      />
                    </div>
                  ...
                  <div className="mt-4 grid gap-5 lg:grid-cols-2">
                    ...
                    <label className="block text-sm font-semibold text-slate-700 lg:col-span-2">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          updateSetting("openingHours", event.target.value)
                        }
                ...
                <div className="mt-8">
                  <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-slate-500">
                        Expanded socialLinks data can be reused across header,
                        footer, contact page, and social buttons.
                      </p>
                    </div>
                    <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600">
                      {activeSocialLinks.length} filled
                    </span>
                  </div>
                  <div className="mt-4 grid gap-5 lg:grid-cols-2">
                    {SOCIAL_LINK_FIELDS.map((field) => (
                      ...
                      >
                        ...
                        <input
                          ...
                          onChange={(event) =>
                            updateSocialLink(field.id, event.target.value)
                          }
            ...
            {activeStep === "type" ? (
              <div>
                ...
                <h3 className="mt-1 text-2xl font-semibold text-slate-950">
                  Choose the website structure
                </h3>
                <p className="mt-2 text-sm text-slate-500">
                  When you choose a type, Sedifex automatically loads the best
                  page list and generator style for that industry.
                </p>
                  ...
                  {WEBSITE_TYPES.map((type) => {
                    ...
                    return (
                      <button
                        key={type.id}
                        type="button"
                        className={`group rounded-2xl border p-4 text-left transition hover:-translate-y-0.5 hover:shadow-lg ${isSelected ? "border-indigo-500 bg-indigo-50 shadow-md ring-4 ring-indigo-100" : "border-slate-200 bg-white hover:border-indigo-200 hover:bg-slate-50"}`}
                        onClick={() => selectWebsiteType(type.id)}
                      >
                        <span
                          className={`inline-flex h-11 w-11 items-center justify-center rounded-2xl text-xl ring-1 ${type.accentClassName}`}
                        >
                          {type.icon}
                        </span>
                        ...
                        {isSelected ? (
                          <span className="mt-3 inline-flex text-xs font-semibold text-indigo-700">
                            Selected smart page set
                          </span>
                        ) : null}
                      </button>
                    );
              ...
              <div>
                <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                  <div>
                    ...
                    <h3 className="mt-1 text-2xl font-semibold text-slate-950">
                      {selectedType.label} pages
                    </h3>
                    <p className="mt-2 text-sm text-slate-500">
                      These page options are based on the selected website type.
                      Change the type to get a different page list.
                    </p>
                  </div>
                  <span className="inline-flex w-fit rounded-full bg-indigo-50 px-3 py-1 text-xs font-bold text-indigo-700">
                    {settings.pages.length} of {smartPageOptions.length}{" "}
                    selected
                  </span>
                </div>
                ...
                <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {smartPageOptions.map((page) => {
                    ...
                    return (
                      <label
                        key={page}
                        className={`flex cursor-pointer items-start gap-3 rounded-2xl border p-4 text-sm font-semibold transition ${isChecked ? "border-indigo-300 bg-indigo-50 text-indigo-800 ring-2 ring-indigo-100" : "border-slate-200 bg-white text-slate-700 hover:border-slate-300 hover:bg-slate-50"}`}
                      >
                        <input
                          type="checkbox"
                          ...
                          onChange={() =>
                            updateSetting(
                              "pages",
                              togglePage(settings.pages, page),
                            )
                          }
                        />
                        <span>
            ...
            {activeStep === "theme" ? (
              <div>
                ...
                <h3 className="mt-1 text-2xl font-semibold text-slate-950">
                  Pick the design style
                </h3>
                <p className="mt-2 text-sm text-slate-500">
                  The live preview changes immediately when a theme is selected.
                </p>
                  ...
                  {THEMES.map((theme) => {
                    ...
                    return (
                      <button
                        key={theme.id}
                        type="button"
                        className={`rounded-3xl border p-4 text-left transition hover:-translate-y-0.5 hover:shadow-lg ${isSelected ? "border-indigo-500 bg-indigo-50 shadow-md ring-4 ring-indigo-100" : "border-slate-200 bg-white hover:border-indigo-200 hover:bg-slate-50"}`}
                        onClick={() => updateSetting("theme", theme.id)}
                      >
                        <div
                          className={`rounded-2xl bg-gradient-to-br ${theme.previewClassName} p-5`}
                        >
                          <div
                            className={`h-3 w-24 rounded-full ${theme.headingClassName}`}
                          />
                          ...
                          <div
                            className={`mt-5 h-8 w-28 rounded-full ${theme.buttonClassName}`}
                          />
                        </div>
              ...
              <div>
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div>
                    ...
                    <h3 className="mt-1 text-2xl font-semibold text-slate-950">
                      Generate first-draft website copy
                    </h3>
                    <p className="mt-2 text-sm text-slate-500">
                      Sedifex can use the business name, type, selected pages,
                      products, services, and gallery to generate a first
                ...
                <div className="mt-5 rounded-2xl border border-indigo-100 bg-indigo-50 p-4 text-sm leading-6 text-indigo-800">
                  ...
                  <span className="font-bold">
                    {settings.businessName || "Wesoamo Foundation"}
                  </span>{" "}
                  {settings.websiteType === "ngo"
                    ? "supports communities through programs, donations, volunteer work, and impact-driven projects."
                    : `helps ${CONTENT_GENERATOR_PROFILE[settings.websiteType].audience} through ${CONTENT_GENERATOR_PROFILE[settings.websiteType].offerPlural}.`}
                </div>
                <div className="mt-6 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                  <button
                    type="button"
                    className="rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-indigo-200 hover:bg-indigo-50"
                    onClick={() => generateContent("homepage")}
                  >
                    <span className="font-bold text-slate-950">
                      Generate homepage content
                    </span>
                    <span className="mt-1 block text-sm text-slate-600">
                      Hero copy and homepage introduction.
                    </span>
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <span className="mt-1 block text-sm text-slate-600">
                      Business story and purpose.
                    </span>
                  </button>
                  <button
                    type="button"
                    className="rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-indigo-200 hover:bg-indigo-50"
                    onClick={() => generateContent("serviceDescriptions")}
                  >
                    <span className="font-bold text-slate-950">
                      Generate service descriptions
                    </span>
                    <span className="mt-1 block text-sm text-slate-600">
                      Services, products, programs, or offers.
                    </span>
                  </button>
                  <button
                    type="button"
                    className="rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-indigo-200 hover:bg-indigo-50"
                    onClick={() => generateContent("seoTitle")}
                  >
                    <span className="font-bold text-slate-950">
                      Generate SEO title
                    </span>
                  ...
                  <button
                    type="button"
                    className="rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-indigo-200 hover:bg-indigo-50"
                    onClick={() => generateContent("tagline")}
                  >
                    <span className="font-bold text-slate-950">
                      Generate website tagline
                    </span>
                    <span className="mt-1 block text-sm text-slate-600">
                      Short brand message for the hero.
                    </span>
                  </button>
                </div>
                <div className="mt-8 grid gap-5">
                  <label className="block text-sm font-semibold text-slate-700">
                    ...
                    <input
                      className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50"
                      value={settings.contentDrafts.seoTitle}
                      onChange={(event) =>
                        updateContentDraft("seoTitle", event.target.value)
                      }
                      placeholder="Business name | Service keywords and location"
                    />
                  </label>
                  <label className="block text-sm font-semibold text-slate-700">
                    Homepage content
                    <textarea
                      className="mt-2 min-h-32 w-full rounded-2xl border border-slate-300 px-4 py-3 outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50"
                      value={settings.contentDrafts.homepage}
                      onChange={(event) =>
                        updateContentDraft("homepage", event.target.value)
                      }
                      placeholder="Generated homepage content will appear here."
                    />
                  </label>
                  <label className="block text-sm font-semibold text-slate-700">
                    About section
                    <textarea
                      className="mt-2 min-h-32 w-full rounded-2xl border border-slate-300 px-4 py-3 outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50"
                      value={settings.contentDrafts.about}
                      onChange={(event) =>
                        updateContentDraft("about", event.target.value)
                      }
                      placeholder="Generated About content will appear here."
                    />
                  </label>
                  <label className="block text-sm font-semibold text-slate-700">
                    Service / product / program descriptions
                    <textarea
                      className="mt-2 min-h-36 w-full rounded-2xl border border-slate-300 px-4 py-3 outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50"
                      value={settings.contentDrafts.serviceDescriptions}
                      onChange={(event) =>
                        updateContentDraft(
                          "serviceDescriptions",
                          event.target.value,
                      ...
                      placeholder="Generated service descriptions will appear here."
                    />
                  </label>
                </div>
              </div>
              ...
              <div>
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div>
                    ...
                    <h3 className="mt-1 text-2xl font-semibold text-slate-950">
                      Control how the website appears online
                    </h3>
                    <p className="mt-2 text-sm text-slate-500">
                      Set the website title, meta description, keywords, and
                      social share image for Google, Facebook, and WhatsApp.
                    </p>
                  </div>
                  <button
                    type="button"
                    className="rounded-2xl bg-slate-950 px-4 py-3 text-sm font-bold text-white transition hover:-translate-y-0.5"
                    onClick={generateSeoSettings}
                  >
                ...
                <div className="mt-6 grid gap-5">
                  <label className="block text-sm font-semibold text-slate-700">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        updateSeoSetting("title", event.target.value)
                      }
                      placeholder="Wesoamo Foundation | Programs, Donations & Volunteer Support"
                    />
                  </label>
                  <label className="block text-sm font-semibold text-slate-700">
                    Meta description
                    <textarea
                      className="mt-2 min-h-24 w-full rounded-2xl border border-slate-300 px-4 py-3 outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50"
                      value={settings.seoSettings.metaDescription}
                      onChange={(event) =>
                        updateSeoSetting("metaDescription", event.target.value)
                      }
                      placeholder="Support Wesoamo Foundation through donations, volunteer programs, and community impact projects."
                    />
                  </label>
                  <label className="block text-sm font-semibold text-slate-700">
                    Keywords
                    <input
                      ...
                      onChange={(event) =>
                        updateSeoSetting("keywords", event.target.value)
                      }
                      placeholder="donations, volunteers, programs, community impact"
                    />
                  </label>
                  <label className="block text-sm font-semibold text-slate-700">
                    Social share image
                    <input
                      className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50"
                      value={settings.seoSettings.socialShareImage}
                      onChange={(event) =>
                        updateSeoSetting("socialShareImage", event.target.value)
                      }
                ...
                <div className="mt-8 grid gap-5 lg:grid-cols-2">
                  <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4">
                      ...
                      <h4 className="mt-1 text-lg font-semibold leading-snug text-blue-700">
                        {seoTitlePreview}
                      </h4>
                    ...
                    <div className="mt-4 overflow-hidden rounded-2xl border border-slate-200 bg-white">
                      ...
                      <div className="p-4">
                        <p className="truncate text-xs uppercase tracking-wide text-slate-500">
                          {customDomain || "sites.sedifex.com"}
                        </p>
            ...
            {activeStep === "payments" ? (
              <div>
                ...
                <h3 className="mt-1 text-2xl font-semibold text-slate-950">
                  Prepare the website for payments
                </h3>
                <p className="mt-2 text-sm text-slate-500">
                  Payment pages now follow the selected website type.
                </p>
                <div className="mt-6 grid gap-4 lg:grid-cols-2">
                  <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span
                        className={`rounded-full px-3 py-1 text-xs font-bold ${hasPaymentPage ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}
                      >
                        {hasPaymentPage ? "Enabled" : "Off"}
                      </span>
                    </div>
                    <button
                      type="button"
                      ...
                      onClick={() =>
                        updateSetting(
                          "pages",
                          togglePage(settings.pages, primaryPaymentPage),
                        )
                      }
                    >
                      {settings.pages.includes(primaryPaymentPage)
                        ? `Remove ${primaryPaymentPage}`
                        : `Enable ${primaryPaymentPage}`}
                    </button>
                  </div>
                  <div className="rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-5">
                    ...
                    <ul className="mt-3 space-y-2 text-sm leading-6 text-slate-600">
                      ...
                      <li>
                        • Show the right payment page for the website type.
                      </li>
                    </ul>
                  </div>
                </div>
              ...
              <div>
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div>
                    ...
                    <h3 className="mt-1 text-2xl font-semibold text-slate-950">
                      Connect a domain after using the free Sedifex URL
                    </h3>
                    <p className="mt-2 text-sm text-slate-500">
                      Every website starts at sites.sedifex.com/slug. Businesses
                      can later connect their own domain with DNS and SSL
                      tracking.
                    </p>
                  </div>
                  <button
                ...
                <div className="mt-6 grid gap-4 lg:grid-cols-2">
                  <div className="rounded-3xl border border-indigo-200 bg-indigo-50 p-5">
                    ...
                    <p className="mt-3 break-all rounded-2xl bg-white px-4 py-3 text-sm font-bold text-indigo-700">
                      {freeUrl}
                    </p>
                    <p className="mt-3 text-sm leading-6 text-indigo-800">
                      This free URL works first and remains available even after
                  ...
                  <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    <input
                      className="mt-3 w-full rounded-2xl border border-slate-300 px-4 py-3 outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50"
                      value={settings.domainSettings.customDomain}
                      onChange={(event) =>
                        updateDomainSetting("customDomain", event.target.value)
                      }
                      placeholder="www.wesoamofoundation.org"
                    />
                    <div className="mt-4 flex flex-wrap gap-2">
                      <span
                        className={`rounded-full border px-3 py-1 text-xs font-bold ${domainStatusConfig.className}`}
                      >
                        Status: {domainStatusConfig.label}
                      </span>
                      <span
                        className={`rounded-full border px-3 py-1 text-xs font-bold ${sslStatusConfig.className}`}
                      >
                        SSL: {sslStatusConfig.label}
                      </span>
                    </div>
                  </div>
                </div>
                <div className="mt-5 grid gap-4 lg:grid-cols-3">
                  <label className="block rounded-3xl border border-slate-200 bg-white p-5 text-sm font-semibold text-slate-700 shadow-sm">
                    Domain connection status
                    <select
                      className="mt-3 w-full rounded-2xl border border-slate-300 px-4 py-3 outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50"
                      value={settings.domainSettings.connectionStatus}
                      onChange={(event) =>
                        updateDomainSetting(
                          "connectionStatus",
                          event.target.value as DomainConnectionStatus,
                        )
                      }
                    >
                  ...
                  <label className="block rounded-3xl border border-slate-200 bg-white p-5 text-sm font-semibold text-slate-700 shadow-sm">
                    ...
                    <select
                      className="mt-3 w-full rounded-2xl border border-slate-300 px-4 py-3 outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50"
                      value={settings.domainSettings.sslStatus}
                      onChange={(event) =>
                        updateDomainSetting(
                          "sslStatus",
                          event.target.value as SslStatus,
                        )
                      }
                    >
                      <option value="not-started">Not started</option>
                      <option value="pending">Pending SSL</option>
                  ...
                  <label className="block rounded-3xl border border-slate-200 bg-white p-5 text-sm font-semibold text-slate-700 shadow-sm">
                    ...
                    <input
                      ...
                      value={settings.domainSettings.dnsTarget}
                      onChange={(event) =>
                        updateDomainSetting("dnsTarget", event.target.value)
                      }
                      placeholder="sites.sedifex.com"
                    />
                  </label>
                </div>
                <div className="mt-6 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    Add these records inside the domain provider dashboard, then
                    return to Sedifex to check connection and SSL.
                  </p>
                  ...
                  {domainFeedback ? (
                    <p className="mt-4 rounded-2xl bg-indigo-50 p-3 text-sm font-medium text-indigo-700">
                      {domainFeedback}
                    </p>
                  ) : null}
                </div>
              ...
              <div>
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div>
                    ...
                    <p className="mt-2 text-sm text-slate-500">
                      Sedifex checks the essentials before a business website
                      goes live.
                    </p>
                  </div>
                  <span
                    className={`rounded-full px-4 py-2 text-sm font-bold ${missingPublishItems.length === 0 ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}
                  >
                    {missingPublishItems.length === 0
                      ? "Ready to publish"
                      : `${missingPublishItems.length} missing`}
                  </span>
                </div>
                <div className="mt-6 grid gap-3 md:grid-cols-2">
                  {publishChecklist.map((item) => (
                    <div
                      key={item.id}
                      className={`rounded-2xl border p-4 ${item.complete ? "border-emerald-200 bg-emerald-50" : "border-amber-200 bg-amber-50"}`}
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div>
                        ...
                        <span
                          className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-bold ${item.complete ? "bg-emerald-600 text-white" : "bg-amber-500 text-white"}`}
                        >
                ...
                {missingPublishItems.length > 0 ? (
                  <p className="mt-4 rounded-2xl border border-indigo-100 bg-indigo-50 p-3 text-sm font-medium text-indigo-700">
                    You can preview now, but complete the missing items before
                    publishing.
                  </p>
                ) : null}
                {feedback ? (
                ...
                <div className="mt-6 grid gap-3 sm:grid-cols-2">
                  ...
                  {!canPublish ? (
                    <p className="sm:col-span-2 -mt-1 text-sm font-medium text-amber-700">
                      Complete the missing checklist items before publishing.
                    </p>
                  ) : null}
                  <button
                ...
                <div className="mt-8 rounded-3xl border border-slate-200 bg-slate-50 p-5">
                  <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-slate-600">
                        These tools appear after publishing so businesses can
                        market the website quickly.
                      </p>
                    </div>
                    <span
                      className={`rounded-full px-3 py-1 text-xs font-bold ${isPublished ? "bg-emerald-100 text-emerald-700" : "bg-slate-200 text-slate-600"}`}
                    >
                      {isPublished ? "Published tools ready" : "Publish first"}
                    </span>
                  </div>
                  {isPublished ? (
                    <div className="mt-5 grid gap-5 lg:grid-cols-[220px_1fr]">
                      <div className="rounded-3xl bg-white p-5 text-center shadow-sm">
                        ...
                        />
                        <p className="mt-3 break-all text-xs font-semibold text-slate-600">
                          {publicWebsiteUrl}
                        </p>
                      </div>
                    ...
                    <div className="mt-5 rounded-2xl bg-white p-4 text-sm font-semibold text-slate-500">
                      Publish the website to unlock QR code, poster download,
                      WhatsApp sharing, Facebook sharing, Instagram caption, and
                      small flyer printing.
                    </div>
                  )}
                  {shareFeedback ? (
                    <p className="mt-4 rounded-2xl bg-indigo-50 p-3 text-sm font-medium text-indigo-700">
        ...
        <aside className="space-y-6">
          <section className="sticky top-6 rounded-3xl border border-slate-200 bg-white p-5 shadow-xl shadow-slate-200/70 md:p-6">
            ...
            <div className="mt-5 rounded-[2rem] border border-slate-200 bg-slate-100 p-3">
              <div
                className={`${previewMode === "mobile" ? "mx-auto max-w-[285px] rounded-[1.75rem]" : "rounded-[1.75rem]"} overflow-hidden border border-slate-900/10 bg-white shadow-sm`}
              >
                <div className="flex items-center gap-1.5 border-b border-slate-200 bg-slate-50 px-4 py-2">
                  <span className="h-2.5 w-2.5 rounded-full bg-red-300" />
                  <span className="h-2.5 w-2.5 rounded-full bg-amber-300" />
                  <span className="h-2.5 w-2.5 rounded-full bg-emerald-300" />
                  <span className="ml-2 truncate rounded-full bg-white px-3 py-1 text-[10px] font-medium text-slate-500">
                    {publicWebsiteUrl}
                  </span>
                </div>
                <div
                  ...
                  style={
                    settings.coverImageUrl
                      ? {
                          backgroundImage: `linear-gradient(135deg, rgba(15, 23, 42, 0.62), rgba(15, 23, 42, 0.2)), url(${settings.coverImageUrl})`,
                ...
                >
                    ...
                    <div className="flex min-w-0 items-center gap-2">
                      {settings.businessLogoUrl ? (
                        <img
                          ...
                          alt="Business logo"
                          className="h-9 w-9 shrink-0 rounded-2xl object-cover ring-1 ring-white/40"
                        />
                      ) : (
                        <span
                          className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl text-lg ring-1 ${selectedType.accentClassName}`}
                        >
                          {selectedType.icon}
                        </span>
                      )}
                      <div className="min-w-0">
                        ...
                        <p className="truncate text-[11px] opacity-75">
                          {settings.tagline || selectedType.label}
                        </p>
                      </div>
                    </div>
                    <span
                  ...
                  <div
                    className={`mt-4 flex gap-2 overflow-hidden text-[11px] ${previewMode === "mobile" ? "flex-wrap" : "flex-nowrap"}`}
                  >
                    {previewPages.map((page) => (
                      <span
                        key={page}
                        className={`rounded-full px-2.5 py-1 ${selectedTheme.mutedSurfaceClassName}`}
                      >
                        {page}
                      </span>
                    ))}
                  ...
                  <div
                    className={`${previewMode === "mobile" ? "grid gap-4" : "grid grid-cols-[1.2fr_0.8fr] gap-4"} mt-6 items-center`}
                  >
                    <div>
                      ...
                      <h4
                        className={`${previewMode === "mobile" ? "text-2xl" : "text-3xl"} mt-2 font-black leading-tight tracking-tight`}
                      >
                    ...
                    <div
                      className={`rounded-3xl p-3 shadow-sm ${selectedTheme.surfaceClassName}`}
                    >
                      ...
                      <div className="mt-3 space-y-2 text-xs">
                        ...
                        <p className="truncate opacity-70">
                          {settings.location || "Business location"}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                      ...
                      <span className="rounded-full bg-white px-3 py-1 text-[11px] font-semibold text-slate-600">
                        Domain:{" "}
                        {
                          DOMAIN_STATUS_CONFIG[
                            settings.domainSettings.connectionStatus
                          ].label
                        }
                      </span>
                    ) : null}
                    ...
                    !isPublished ? (
                      <span className="text-[11px] font-semibold text-slate-400">
                        Opening hours, SEO, domain, and social links will show
                        here.
                      </span>
                    ) : null}
                  </div>
            ...
            {copyFeedback ? (
              <p className="mt-3 rounded-2xl bg-indigo-50 p-3 text-sm font-medium text-indigo-700">
                {copyFeedback}
              </p>
            ) : null}
          </section>
