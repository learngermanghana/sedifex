...
function mergeSeoSettings(...sources: unknown[]): SeoSettings {
  ...
  sources.forEach(source => {
    const record = getRecord(source)
    ;(['title', 'metaDescription', 'keywords', 'socialShareImage'] as Array<keyof SeoSettings>).forEach(key => {
      const value = record[key]
      if (typeof value === 'string' && value.trim()) merged[key] = value.trim()
    })
  })
...
export default function WebsiteBuilder() {
  ...
  const previewUrl = useMemo(() => `https://sites.sedifex.com/${settings.slug || 'your-business'}`, [settings.slug])
  ...
  const completionItems = [
    ...
    { label: 'SEO settings', complete: stepCompletion.seo },
    { label: 'Payment page', complete: stepCompletion.payments },
    { label: 'Domain setup', complete: stepCompletion.domain },
  ]
  ...
  function updateDomainSetting(key: keyof DomainSettings, value: string) { setSettings(previous => ({ ...previous, domainSettings: { ...previous.domainSettings, [key]: key === 'customDomain' ? normalizeDomain(value) : value } })) }
  ...
  async function copyDnsRecords() {
    const text = dnsRecords.map(record => `${record.type}\t${record.host}\t${record.value}`).join('\n')
    try {
      await navigator.clipboard.writeText(text)
      setDomainFeedback('DNS records copied.')
    } catch (copyError) {
      console.error('[website-builder] Unable to copy DNS records', copyError)
      setDomainFeedback('Could not copy DNS records. You can copy them manually from the table.')
    }
    window.setTimeout(() => setDomainFeedback(null), 2500)
  }
  ...
  async function saveSettings(nextStatus: StoredWebsiteStatus) {
    ...
    if (nextStatus === 'published' && normalizedPages.length === 0) { setError('Select at least one page before publishing.'); return }
    ...
    try {
      const payload: WebsiteBuilderSettings = {
        ...
        domainSettings: { ...mergeDomainSettings(settings.domainSettings), verificationToken },
        status: nextStatus,
      }
  ...
  async function copyWebsiteLink() {
    try { await navigator.clipboard.writeText(previewUrl); setCopyFeedback('Website link copied.') } catch (copyError) { console.error('[website-builder] Unable to copy website link', copyError); setCopyFeedback('Could not copy link. You can copy it from the preview URL.') }
  ...
  return (
    <PageSection title="Website Builder" subtitle="Control your business website from Sedifex: setup, pages, theme, preview, SEO, domain, content, and publishing from one place." className="pt-8 md:pt-10" actions={<div className="flex flex-wrap items-center gap-3"><span className={`inline-flex items-center gap-2 rounded-full border px-3 py-2 text-sm font-semibold ${statusConfig.className}`}><span className={`h-2.5 w-2.5 rounded-full ${statusConfig.dotClassName}`} />{statusConfig.label}</span><a className="button button--ghost" href={previewUrl} target="_blank" rel="noreferrer">Preview website</a></div>}>
      <form onSubmit={handleSubmit} className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(420px,0.72fr)]">
        <div className="space-y-6">
          <section className="overflow-hidden rounded-3xl border border-slate-200 bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-900 p-6 text-white shadow-sm"><div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between"><div><p className="text-sm font-semibold uppercase tracking-[0.22em] text-cyan-200">Business website control center</p><h3 className="mt-3 text-2xl font-bold tracking-tight md:text-3xl">Prepare content, SEO, domain, and previews before publishing</h3><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-200">Start with the free Sedifex URL, then connect a custom domain with DNS records and SSL status tracking.</p></div><div className="rounded-2xl border border-white/10 bg-white/10 p-4 text-sm shadow-xl backdrop-blur"><p className="text-slate-300">Current website</p><p className="mt-1 font-semibold text-white">{settings.businessName || 'Loading business…'}</p><p className="mt-3 break-all rounded-xl bg-white/10 px-3 py-2 text-cyan-100">{previewUrl}</p></div></div></section>
