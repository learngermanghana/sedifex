...
import {
  deriveLastEventType,
  deriveOnlineOrderStatusFromBooking,
  deriveReportPaymentFields,
  normalizePaymentStatus as normalizeCanonicalPaymentStatus,
...
function normalizePaymentStatusValue(value: unknown, fallback = 'pending'): string {
  const safeFallback = ['pending', 'partial', 'paid', 'awaiting_verification'].includes(fallback)
    ? fallback
    : 'pending'

  return normalizeCanonicalPaymentStatus(
    value,
    safeFallback as 'pending' | 'partial' | 'paid' | 'awaiting_verification',
  )
}

function normalizePaymentStatus(data: Record<string, unknown>, payment: Record<string, unknown>): string {
  const explicitStatus = firstStringValue(
    data.paymentStatus,
    data.payment_status,
...
function normalizeBookingForm(data: Record<string, unknown>): BookingFormState {
  ...
  const status = normalizeBookingStatusValue(
    firstStringValue(data.bookingStatus, data.booking_status, data.status),
    'confirmed',
  )
  ...
  return {
    ...
    bookingDate: normalizeDateInput(
      firstStringValue(data.bookingDate, data.date, booking.preferredDate, booking.date, booking.startAt),
    ),
    bookingTime: normalizeTimeInput(
      firstStringValue(data.bookingTime, data.time, booking.preferredTime, booking.time, booking.startAt),
    ),
    ...
    depositAmount: firstStringValue(
      data.depositAmount,
      data.depositPaid,
      data.amountPaid,
...
const bookingAppsScriptUrl =
  'https://script.google.com/macros/s/AKfycbxl0IdT746Z_yL2LJbAOKi0wsn3iNct4H1omFYWaxq8nzzI7rc_cebfxXcMxMydtvO4Eg/exec'
...
async function syncBookingToSheet(payload: BookingSheetPayload) {
  const res = await fetch(bookingAppsScriptUrl, {
    method: 'POST',
    headers: {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const orderQueries = [
        ...
        ...(paymentReference
          ? [
              ...
              query(
                collection(db, 'stores', storeId, 'integrationOrders'),
      ...
      await syncBookingToSheet({
        ...
        last_event_type: lastEventType,
        lastEventType,
        source: isConfirmedPaid
          ? 'sedifex-booking-confirmed-paid'
          : isConfirmedPartial
            ? 'sedifex-booking-partial-payment'
            : 'sedifex-booking-update',
      })
    ...
    <main className="page booking-editor-page">
      <section className="card booking-editor-page__card stack gap-3">
        <header className="stack gap-1">
          ...
          <p className="form__hint">
            Update the booking status in this form, then click <strong>Save changes</strong>. Sedifex will
            save the update and send the customer email when notifications are enabled.
          </p>
        </header>
          ...
          >
            ...
            <label>
              ...
              <input
                ...
                onChange={event => setForm(prev => ({ ...prev, bookingTime: event.target.value }))}
              />
            </label>
            ...
            <label>
              ...
              <input
                ...
                onChange={event => setForm(prev => ({ ...prev, preferredBranch: event.target.value }))}
              />
            </label>
            ...
            <label>
              ...
              <input
                ...
                onChange={event => setForm(prev => ({ ...prev, paymentAmount: event.target.value }))}
              />
            </label>
            ...
            <label className="booking-editor-page__notes">
              <span>Notes</span>
              <textarea
                value={form.notes}
